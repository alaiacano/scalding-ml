...
object DataSources {
  ...
  /////////////////////// DSP STUFF
  def dspNoisey   : Seq[(Long, Double)] = {
    Source.fromURL(getClass.getResource("/dsp/noisey.tsv"))
          .mkString
          .split("\n")
          .map { line =>
            val ln = line.split("\t")
            (line(0).toLong, line(1).toDouble)
          }
  }

  def dspFiltered   : Seq[(Long, Double)] = {
    Source.fromURL(getClass.getResource("/dsp/filtered.tsv"))
          .mkString
          .split("\n")
          .map { line =>
            val ln = line.split("\t")
