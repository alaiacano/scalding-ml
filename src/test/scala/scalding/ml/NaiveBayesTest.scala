...
class NaiveBayesJob(args : Args) extends Job(args) {
...
class NaiveBayesTest extends Specification {
  import Dsl._
  ...
  val scikitResults = Map[Int,Int](
    ...
    30 -> 0,33 -> 0,36 -> 0,39 -> 0,42 -> 0,45 -> 0,48 -> 0,51 -> 1,54 -> 1,
    57 -> 1,60 -> 1,63 -> 1,66 -> 1,69 -> 1,72 -> 1,75 -> 1,78 -> 1,81 -> 1,
    84 -> 1,87 -> 1,90 -> 1,93 -> 1,96 -> 1,99 -> 1,102 -> 2,105 -> 2,108 -> 2,
    111 -> 2,114 -> 2,117 -> 2,120 -> 2,123 -> 2,126 -> 2,129 -> 2,132 -> 2,
    135 -> 2,138 -> 2,141 -> 2,144 -> 2,147 -> 2
  )

  noDetailedDiffs() //Fixes an issue with scala 2.9
  "A GaussianNB model" should {
    TUtil.printStack {
    JobTest(new scalding.ml.NaiveBayesJob(_))
